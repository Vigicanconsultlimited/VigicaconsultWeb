...
const styles = `
  ...
  /* ── Social Icons (static, always visible) ── */
  .tp-card-socials {
    position: static;
    background: transparent;
    transform: none;
...
// ─── Member Card ──────────────────────────────────────
// FIX: Component uses its own index-based animation; removed unused prop destructuring
...
function TeamPage() {
  ...
  const fetchTeamByCategory = async () => {
    try {
      ...
      // FIX: Actually set the error state so the UI shows it
        ...
        <div className="tp-main">
              ...
              {categorizedTeam.map((item) => {
                ...
                return (
                  <div key={categoryId} className="tp-cat-section">
                    ...
                    {/* Category Members */}
                    <AnimatePresence>
                      {!isCollapsed && item.members?.length > 0 && (
                        <motion.div
                        ...
                        >
                          {/* FIX: Use tp-grid class and pass index prop correctly */}
                          <motion.div
                            className="tp-grid"
                    ...
                    {!isCollapsed && (!item.members || item.members.length === 0) && (
          ...
          {!loading && !error && viewMode === "all" && allMembers.length > 0 && (
            <motion.div
              ...
              animate="visible"
            >
              {/* FIX: pass index prop so stagger animation works */}
